...
| Global note | Top of "Analyse par dimensions" view, above the 3 cards | **Added 2026-09-11, post-Epic 2**, on explicit PM request: plain average of the 3 dimension notes (1 decimal, French comma notation), same typographic hierarchy as V1's retired "Note finale" (`font-display text-5xl font-semibold`, small-caps label above). Purely client-computed, no new API call. Partially reverses the original decision to drop a global/averaged score — but the "point faible" (weakest-dimension flagging) concept it was originally bundled with stays dropped: this is a neutral summary number only. |
| Dimension card (Objectif / Contexte / Contraintes) | "Analyse par dimensions" view, one row of 3 (`sm:grid-cols-3`) | Collapsed by default, all three treated identically (no dimension is visually promoted as "the weak one" — see `.memlog.md` decision on Point 1; the global note above does not change this — it never singles out a dimension). Shows the note (0-10) and the palier label (`DESIGN.md.Components` status pill) when collapsed. Click anywhere on the card to expand/collapse (`aria-expanded`, see Accessibility Floor). |
