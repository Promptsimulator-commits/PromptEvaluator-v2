...
- **Rule:** exactement ces quatre routes, ces formes de requête/réponse :
  ...
  - `POST /api/analyze-dimensions` *(remplace `/api/analyze`)* : `{ prompt: string, scoreResults?: { criterion: string, passed: boolean }[][] }` → `{ dimensions: { name: "objectif" | "contexte" | "contraintes", note: number, palier: "absent" | "a-clarifier" | "clair" | "tres-clair", explanation: string, rewriteSuggestion: string, correctionApplied: boolean, correctionDetail?: string }[] }` — `scoreResults` est le tableau des résultats de `/api/score` pour les N réponses (une entrée par réponse), utilisé pour calculer la correction comportementale (AD-4b). Absent ou vide (N=1) → `correctionApplied: false` sur toutes les dimensions.
...
- **Rule:** `/api/analyze-dimensions` calcule la correction de chaque dimension à partir de `scoreResults` selon ce barème fixe (3 niveaux, mêmes points pour les 3 dimensions) :
  - **Contraintes** : taux de réponses en défaut sur au moins un critère (fixe ou extrait) → 0% = faible (-0), 1-33% = modérée (-2), >33% = forte (-4)
  - **Objectif** : divergence qualitative entre réponses sur le sujet/angle/niveau de détail → même barème faible/modérée/forte
  - **Contexte** : divergence qualitative entre réponses sur le ton → même barème faible/modérée/forte
  - La correction s'applique **avant** la traduction en palier (0 = Absent, 1-5 = À clarifier, 6-8 = Clair, 9-10 = Très clair). Si `scoreResults` est absent ou contient une seule réponse (N=1), `correctionApplied` est `false` sur les 3 dimensions et aucune correction n'est appliquée.
